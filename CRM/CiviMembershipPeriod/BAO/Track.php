...
class CRM_CiviMembershipPeriod_BAO_Track extends CRM_CiviMembershipPeriod_DAO_Track {
   ...
   *
   * @return CRM_CiviMembershipPeriod_DAO_Track
   */
  public static function create($params) {
    $hook = empty($params['id']) ? 'create' : 'edit';
    CRM_Utils_Hook::pre($hook, 'PeriodTrack', CRM_Utils_Array::value('id', $params), $params);

    $dao = new CRM_CiviMembershipPeriod_DAO_Track();
    $dao->copyValues($params);
    $dao->save(); 
    CRM_Utils_Hook::post($hook, 'PeriodTrack', $dao->id, $dao);
    return $dao;
  }
   ...
   *
   * @return object CRM_CiviMembershipPeriod_BAO_Item object on success, null otherwise
   * @access public
   * @static
   */
  static function retrieve(&$params, &$defaults) {
    $item = new CRM_CiviMembershipPeriod_DAO_Track();
 ...
 /**
   * For Getting Data From civimembershipperiod_track by Contact Id .
   * 
   */

  static function getUsageByContact($id) {
    return CRM_CiviMembershipPeriod_BAO_Track::getPeriod(NULL, $id, NULL);
  }

  static function getUsageByOrg($id) {
    return CRM_CiviMembershipPeriod_BAO_Track::getPeriod(NULL, NULL, $id);
  }
 

  static function getPeriod($id = NULL, $cid = NULL, $orgid = NULL) {
    require_once 'CRM/CiviMembershipPeriod/Utils.php';
...
SELECT * ";
    ...
    $from = " FROM civimembershipperiod_track AS t ";

    if ($orgid) {
      // $sql .= ", i.code ";
      $where = " LEFT JOIN civicrm_contact AS c ON (c.id = t.contact_id) ";
      $where .= " WHERE t.organization_id = " . CRM_Utils_Type::escape($orgid, 'Integer');
    }
    else {
      if ($cid) {
        // $sql .= ", civicrm_contact AS c ";
        $where = " LEFT JOIN civicrm_contact AS c ON (c.id = t.contact_id) ";
        $where .= " WHERE t.contact_id = " . CRM_Utils_Type::escape($cid, 'Integer');
      } 
    }

    $orderby = " ORDER BY t.membership_id ASC";
    ...
    while ($dao->fetch()) {
      ...
      $row['membership_name'] = $dao->membership_name;
      ...
      if ($row['entity_table'] == 'civicrm_participant') {
        ...
        $events = CRM_CiviMembershipPeriod_Utils::getEvents();
  ...
  /**
   * Function to delete Time/Period track
   *
   * @param  int $trackID ID of the Period track to be deleted.
   *
   * @access public
   * @static
